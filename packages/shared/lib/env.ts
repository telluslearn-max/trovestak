 ...
 */
export function validateEnv<T extends string>(requiredVars: T[]): Record<T, string> {
    ...
    return Object.fromEntries(requiredVars.map((key) => [key, process.env[key]!])) as Record<T, string>;
}
