...
export function transformProduct(csvRow, header) {
...
function getCategoryFromNav(navCategory, productName) {
    const categoryMap = {
        'Mobile': 'mobile',
        'Computing': 'computing',
        'Audio': 'audio',
...
function getSubcategoryFromName(productName) {
    const lowerName = productName.toLowerCase();
    // Mobile
    if (lowerName.includes('pro max') || lowerName.includes('ultra'))
        return 'flagship-phones';
    if (lowerName.includes('pro') || lowerName.includes('plus'))
        return 'mid-range-phones';
    if (lowerName.includes('se') || lowerName.includes('mini'))
        return 'budget-phones';
    if (lowerName.includes('ipad'))
        return 'ipad';
    if (lowerName.includes('tablet'))
        return 'android-tablets';
    // Audio
    if (lowerName.includes('earbud') || lowerName.includes('airpod'))
        return 'wireless-earbuds';
    if (lowerName.includes('headphone') || lowerName.includes('wh-'))
        return 'over-ear-headphones';
    if (lowerName.includes('speaker') || lowerName.includes('jbl'))
        return 'bluetooth-speakers';
    if (lowerName.includes('soundbar'))
        return 'soundbars';
    // Computing
    ...
    if (lowerName.includes('monitor') || lowerName.includes('display'))
        return 'monitors';
    if (lowerName.includes('keyboard') || lowerName.includes('mouse') || lowerName.includes('webcam'))
        return 'peripherals';
    if (lowerName.includes('router') || lowerName.includes('switch') || lowerName.includes('network'))
        return 'networking';
    if (lowerName.includes('printer') || lowerName.includes('scanner'))
        return 'printers';
    if (lowerName.includes('ssd') || lowerName.includes('hdd') || lowerName.includes('storage'))
        return 'storage';
    if (lowerName.includes('ups') || lowerName.includes('back-ups'))
        return 'power-ups';
    // Wearables
    ...
    if (lowerName.includes('fitness') || lowerName.includes('fitness band'))
        return 'fitness-trackers';
    // Gaming
    if (lowerName.includes('ps5') || lowerName.includes('playstation'))
        return 'playstation';
    if (lowerName.includes('xbox'))
        return 'xbox';
    if (lowerName.includes('nintendo') || lowerName.includes('switch'))
        return 'nintendo-switch';
    if (lowerName.includes('controller') || lowerName.includes('headset') || lowerName.includes('gaming'))
        return 'gaming-gear';
    // Cameras
    if (lowerName.includes('drone'))
        return 'drones-gimbals';
    if (lowerName.includes('gimbal'))
        return 'drones-gimbals';
    if (lowerName.includes('action camera') || lowerName.includes('gopro'))
    ...
    if (lowerName.includes('tv') || lowerName.includes('television'))
        return 'smart-tvs';
    if (lowerName.includes('light') || lowerName.includes('bulb'))
        return 'smart-lights';
    if (lowerName.includes('appliance') || lowerName.includes('fridge') || lowerName.includes('washer'))
        return 'home-appliances';
    return '';
}
function determineBadge(productName) {
    const lowerName = productName.toLowerCase();
...
// Process CSV file
export async function processCSV(inputPath, outputDir, onProgress) {
    // Read CSV file
...
// Generate index file
export function generateIndex(outputDir) {
    const categories = {};
