...
export function detectVariants(productName) {
    const variants = [];
...
export function extractStorage(productName) {
    const storage = [];
    const lowerName = productName.toLowerCase();
    for (const pattern of STORAGE_PATTERNS) {
        const match = lowerName.match(pattern);
...
export function extractRAM(productName) {
...
export function extractChip(productName) {
...
export function generateShortName(productName) {
